...
class MessageSettingController extends Controller
{
    ...
    {
        $query = MessageSetting::with('createdBy:id,name');
        
        // Search functionality
        if ($request->has('search') && $request->search != '') {
            $search = $request->search;
            $query->where(function($q) use ($search) {
                $q->where('message_type', 'like', '%' . $search . '%')
    ...
    public function store(Request $request)
    {
        $validator = Validator::make($request->all(), [
            ...
            'whatsapp_api' => 'nullable|string|max:500',
    ...
    public function show($id)
    {
        ...
        $data['token'] = $messageSetting->getRawOriginal('token');
    ...
    public function update(Request $request, $id)
    {
        ...
        $validator = Validator::make($request->all(), [
            'message_type' => 'sometimes|required|string|max:50',
            'whatsapp_api' => 'nullable|string|max:500',
            'token' => 'nullable|string|max:500',
            'secret_key' => 'nullable|string|max:500',
            'template_name' => 'nullable|string|max:255',
